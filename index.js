...
document.addEventListener('DOMContentLoaded', () => {
    ...
    $('#shuqian-item').addEventListener('dragend', () => {
        setTimeout(() => {
            if (!droppedInside) {
                window.location.href = 'https://www.gangqinpu.com/search/';
            }
            droppedInside = false;
        }, 50);
    });
    ...
    $('#chongchonglink').onclick = () => open("https://gangqinpu.com/");
    $('#tampermonkeylink').onclick = () => open("https://update.greasyfork.org/scripts/457019/虫虫钢琴铺面查看脚本.user.js");

    document.querySelectorAll('linkitem').forEach(e => {
        e.onclick = () => open(e.getAttribute('href'));
    });
